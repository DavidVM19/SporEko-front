...
const points = [
  ...
  { name: 'Bordeaux', lat: 44.837789, lng: -0.579180},
...
const haveLocation = () => {
  navigator.geolocation.getCurrentPosition(function(position) {
    return ( setShowLocation(`${position.coords.latitude} + ${position.coords.longitude}`));
  }), (error: any) => {
    ...
    <div className="header">
      ...
      {showSearch && <SearchBar />}
        ...
        <div className='geolocation'>
          <button type='button' onClick={haveLocation}>Ma position</button>
          <input type="input" value={showLocation} readOnly></input>
          <p>Près de chez vous</p>
        </div>
        <div>
          <input id="searchHome" type="search" value={searchText} onChange={(e) => setSearchText(e.target.value)}/>
        </div>
        <div>

...
console.log(allSports.length)
  ...
  return (
    <div className='searchBar'>
                          ...
                          <div className='genderFilter'>
                            <div className='genderFilterDetail'>
                              ...
                              {allGenders.map((gender: IGender, index: number) => {
                                  return (
                                      <a key={index}> {gender.adult_name}</a>
                                  )
                              })}
                            </div>
                            <div className='genderFilterDetail'>
                              ...
                              {allGenders.map((gender: IGender, index: number) => {
                                  return (
                                      <a key={index}> {gender.child_name}</a>
                                  )
                              })}
                            </div>
                          </div>
      ...
      <button type="button" className='btnAllSports' onClick={() => setShowAllSport(!showAllSport)}>{showAllSport ? 'Top 15 des sports' : 'Tous les sports'}</button>
    </div>
  );
};
